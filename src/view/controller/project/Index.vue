<script setup lang="ts">
    ...
    const setShowData = async () => {
        Project.show(search_data).then((res)=>{
            if(res.data.code === 0){
    ...
    const add = (table:string,id:number) => {
        ...
        router.push({path:'/project/add'})
    }

    const mod = (table:string,id:number) => {
        setData(id)
        console.log('table: '+table+ ' id: '+ id)
        router.push({path:'/project/add',query:{id: id}})
    }
