<script setup lang="ts">
    ...
    const table = ref('carousel')
    const dialogFormVisible = ref(false)
    const tableData = ref<DataList>()
    const formLabelWidth = ref(180)
    const form = reactive({id:0,cid:'',title:'',img:'',url:'https://www.huanrang.com',opentype:'navigate',sort:''}) 
    ...
    const mod = (table:string,id:number)=>{
        fileList.value=[]
         Carousel.get(id).then((res)=>{
            console.log(res.data.data)
            form.id = JSON.parse(JSON.stringify(res.data.data)).id
            form.title = JSON.parse(JSON.stringify(res.data.data)).title
            form.img = JSON.parse(JSON.stringify(res.data.data)).img
            form.url = JSON.parse(JSON.stringify(res.data.data)).url
            form.opentype = JSON.parse(JSON.stringify(res.data.data)).opentype
            form.cid = JSON.parse(JSON.stringify(res.data.data)).cid
    ...
    const search = () => {
        Carousel.show(formSearch).then((res)=>{
            console.log(res)
            tableData.value = JSON.parse(JSON.stringify(res.data.data))
        })
    }
...
<template>
    ...
    <el-dialog v-model="dialogFormVisible" :title="form.id? '修改轮播图' : '新增轮播图'">
