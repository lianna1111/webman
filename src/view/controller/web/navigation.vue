<script setup lang="ts">
    ...
    const search = () => {
        WebNav.show(formSearch).then((res)=>{
            console.log(res)
            tableData.value = JSON.parse(JSON.stringify(res.data.data))
        })
    }
...
<template>
     <el-container>
        <el-header class="flex justify-between bg-white border-b py-4">
            ...
            <div>
                <el-button type="primary" plain   @click="ElMessage.warning('您无修改权限')">新增</el-button>
