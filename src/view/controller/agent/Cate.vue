<script setup lang="ts">
    ...
    import AgentCate from '@/api/AgentCate'
    ...
    const rules = ref()
    let form = reactive({title:'',sign:'',pic:'',sort:'',create_time:''})
    ...
    const setShowData = async () => {
        AgentCate.show(search_data).then((res)=>{
            if(res.data.code === 0){
    ...
    const submit = () => {
        if(fileList.value.length){
            ...
            // console.log(fileList.value[0].response.data.url,'url');
        }
    
        AgentCate.save(form).then((res)=>{
            if(res.data.code === 0){
...
<template>
    ...
    <!-- action form -->
    <el-dialog v-model="dialogFormVisible" title="新增类别" width="700px">
