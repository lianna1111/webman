<script setup lang="ts">
    ...
    const table = ref('tabbar')
    const dialogFormVisible = ref(false)
    const tableData = ref<DataList>()
    const formLabelWidth = ref(180)
    const form = reactive({id:0,title:'',pic:'',active_pic:'',type:0,url:'',is_valid:1}) 
    ...
    const mod = (table:string,id:number)=>{
         Tabbar.get(id).then((res)=>{
            ...
            form.active_pic = JSON.parse(JSON.stringify(res.data.data)).active_pic
            form.type = JSON.parse(JSON.stringify(res.data.data)).type
            form.url = JSON.parse(JSON.stringify(res.data.data)).url
            form.is_valid = JSON.parse(JSON.stringify(res.data.data)).is_valid
    ...
    const search = () => {
        Tabbar.show(formSearch).then((res)=>{
            console.log(res)
            tableData.value = JSON.parse(JSON.stringify(res.data.data))
        })
    }
...
<template>
    ...
    <el-dialog v-model="dialogFormVisible" :title="form.id? '修改导航栏' : '新增导航栏'">
