<script setup lang="ts">
    ...
    onMounted(async () => {
        await ActivityCate.show({role_id:8}).then((res)=>{
    ...
    const search = () => {
        ActivityCate.show(formSearch).then((res)=>{
            console.log(res)
            tableData.value = JSON.parse(JSON.stringify(res.data.data))
        })
    }
...
<template>
    ...
    <el-dialog v-model="dialogFormVisible" :title="form.id? '修改活动类别' : '新增活动类别'">
