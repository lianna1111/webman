<script setup lang="ts">
    ...
    const formSearch = reactive({keyword:'',start_time:'',end_time:'',is_auth:'',parent_id:'',page:1,pagesize:10,role_id:19})
    const formMod = reactive({uid:0,username:'',realname:'',nickname:'',mobile:0,email:'',is_auth:0,role_id:'',charge_type:1,charge_value:0,avatar:'',idcard:'',password:'',lock_type:0,lock_time:0,lock_date_type:1,lock_reason:''})
    const formExport = reactive({field:['AccountID','UserName','RealName','Gender','Mobile','Idcard','Birth','RegTime'],is_auth:1,start_time:'',end_time:'',orderby:1})
    const exportField = ref([{title:'账号ID',value:'AccountID',checked:true},{title:'用户名',value:'UserName',checked:true},{title:'真实姓名',value:'RealName',checked:true},{title:'性别',value:'Gender',checked:true},{title:'手机号码',value:'Mobile',checked:true},{title:'身份证号',value:'Idcard',checked:true},{title:'出生日期',value:'Birth',checked:true},{title:'注册时间',value:'RegTime',checked:true}])
    const fields = ref([])
    ...
    // 改密
    const setPassword = (uid:number) => {
        formMod.uid = uid
        User.get(uid).then((res)=>{
            if(res.data.code === 0){
                formMod.mobile = JSON.parse(JSON.stringify(res.data.data)).mobile
                formMod.username = JSON.parse(JSON.stringify(res.data.data)).username
                formMod.nickname = JSON.parse(JSON.stringify(res.data.data)).realname
                formMod.realname = JSON.parse(JSON.stringify(res.data.data)).realname
                formMod.idcard = JSON.parse(JSON.stringify(res.data.data)).idcard
            }
        })
    ...
    // 用户信息
    const setUserInfo = (uid:number) => {
        formMod.uid = uid
        User.get(uid).then((res)=>{
            if(res.data.code === 0){
                formMod.mobile = JSON.parse(JSON.stringify(res.data.data)).mobile
                formMod.username = JSON.parse(JSON.stringify(res.data.data)).username
                formMod.nickname = JSON.parse(JSON.stringify(res.data.data)).nickname
                formMod.is_auth = JSON.parse(JSON.stringify(res.data.data)).is_auth
                formMod.realname = JSON.parse(JSON.stringify(res.data.data)).realname
                formMod.idcard = JSON.parse(JSON.stringify(res.data.data)).idcard
                formMod.role_id = JSON.parse(JSON.stringify(res.data.data)).role_id
            }
        })
    ...
    const submitExport = () => {
        ElLoading.service()
        // User.exports(formExport).then((res)=>{
        //     console.log(res)
        //     if(res.data.code === 0){
...
<template>
     <el-container>
        <el-header class="flex justify-between bg-white border-b py-4">
            ...
            <div>
                <el-button type="primary" plain @click="dialogUserReg = true">注册会员</el-button>
            </div>
        </el-header>
                ...
                <el-table-column label="操作">
                    <template #default="scope">
                        <el-dropdown>
                            ...
                            <template #dropdown>
                            <el-dropdown-menu>
                                <el-dropdown-item @click="setPassword(scope.row.uid)">改密</el-dropdown-item>
                                <el-dropdown-item @click="setUserInfo(scope.row.uid)">个人</el-dropdown-item>
                                <el-dropdown-item @click="setCharge(scope.row.uid)">充值</el-dropdown-item>
                                <el-dropdown-item @click="setLocked(scope.row.uid)">锁定</el-dropdown-item>
                                <el-dropdown-item @click="setUnLocked(scope.row.uid)">解锁</el-dropdown-item>
                                <el-dropdown-item @click="setDel(scope.row.uid)">删除</el-dropdown-item>
                            </el-dropdown-menu>
                            </template>
                        </el-dropdown>
        ...
        <div class="flex justify-between items-center bg-white p-3">
            <element-button-group :items="[]" @exports="exports" />
    ...
    <!-- 注册会员 表单 -->
    <el-dialog v-model="dialogUserReg" title="注册会员">
        <el-form size="large" :model="formMod" :label-width="formLabelWidth">
            ...
            <el-form-item label="手机号码">
                <el-col :span="12">
                    <el-input v-model="formMod.mobile" class="text-gray-500" />
                </el-col>
            </el-form-item>
            <el-form-item label="姓名" v-if="!formMod.is_auth">
                <el-col :span="12">
                    <el-input v-model="formMod.realname" autocomplete="off" />
                </el-col>
            </el-form-item>
            <el-form-item label="身份证号" v-if="!formMod.is_auth">
                <el-col :span="12">
        ...
        <template #footer>
            <span class="dialog-footer">
                <el-button @click="dialogUserReg = false">取消</el-button>
                <el-button type="primary" @click="submitUserReg" class="bg-blue-400">确认注册</el-button>
            </span>
        </template>
    </el-dialog>
    ...
    <el-dialog v-model="dialogUserInfo" title="修改用户信息">
        <el-form size="large" :model="formMod" :label-width="formLabelWidth">
            ...
            <el-form-item label="角色">
                <el-col :span="12">
                    <el-cascader v-model="formMod.role_id" :options="roleData" @change="changeRole" />
                </el-col>
            </el-form-item>
        </el-form>
        <template #footer>
    ...
    <!-- 导出 表单 -->
    <el-dialog v-model="dialogExport" title="导出会员列表">
        <el-form size="large" :model="form" :label-width="formLabelWidth">
            ...
            <el-form-item label="实名">
                <el-col :span="13">
                    <el-radio-group v-model="formExport.is_auth">
                        <el-radio :label="1">全部</el-radio>
                        <el-radio :label="2">已实名</el-radio>
                        <el-radio :label="3">未实名</el-radio>
                    </el-radio-group>
                </el-col>
            </el-form-item>
