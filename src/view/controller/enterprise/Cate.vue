<script setup lang="ts">
    ...
    import EnterpriseCate from '@/api/EnterpriseCate'

    const table = ref('enterprise_cate')
    const id = ref(0)
    const pid = ref(0)
    const level = ref(1)
    ...
    const setShowData = async () => {
        EnterpriseCate.show(search_data).then((res)=>{
            if(res.data.code === 0){
    ...
    const setData = (id:number) => {
        if(id > 0){
            EnterpriseCate.get(id).then((res)=>{
                if(res.data.code === 0){
    ...
    const submit = () => {
        EnterpriseCate.save(form).then((res)=>{
            if(res.data.code === 0){
...
<template>
    ...
    <!-- action form -->
    <el-dialog v-model="dialogFormVisible" title="新增认证类别" width="700px">
